...
void loop() {
 ...
 float f;
  ...
  else if(angleCounter == 4){
    powerD8 = f;
  }
  else if(angleCounter == 5){
    powerD9 = f;
  }
  else if(angleCounter == 6){
    powerD10 = f;
    //reset the angle counter
    angleCounter = 0;
    moveArmToAngles(baseAngle, upperArmAngle, lowerArmAngle, powerD8, powerD9, powerD10);
  }
}


void moveArmToAngles(float baseAngle, float upperArmAngle,
     float lowerArmAngle, float powerD8, float powerD9, float powerD10){
  Serial.print("PowerD8:");Serial.println(powerD8);
  digitalWrite(8, powerD8);
  Serial.print("PowerD9:");Serial.println(powerD9);
  analogWrite(9, powerD9);
  Serial.print("PowerD10:");Serial.println(powerD10);
  ...
  Serial.print("Base Step Number:");Serial.println(baseStepNumber);
  Serial.print("Upper Arm Angle:");Serial.println(upperArmAngle);
  int upperArmStepNumber = ( (abs(upperArmAngle)/360) * upperArmActualStepsPerRevolution ) + 0.5;
  //need this because of the abs value function, which is needed for proper rounding
  if (upperArmAngle < 1){
    upperArmStepNumber *= -1;
  }
  Serial.print("Upper Arm Step Number:");Serial.println(upperArmStepNumber);
  Serial.print("Lower Arm Angle:");Serial.println(lowerArmAngle);
  int lowerArmStepNumber = ( (abs(lowerArmAngle)/360) * lowerArmActualStepsPerRevolution ) + 0.5;
  //need this because of the abs value function, which is needed for proper rounding
  if (lowerArmAngle < 1){
    lowerArmStepNumber *= -1;
  }
  Serial.print("Lower Arm Step Number:");Serial.println(lowerArmStepNumber);
