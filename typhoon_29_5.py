...
class Typhoon():
    def __init__(self, port: str, length_upper_arm: int = 215, length_lower_arm: int = 250, distance_tool: int = 165, distance_z: int = 77, height_from_ground: int = 0, output: bool = False):
        try:
    ...
    def angles_from_coordinates(self, x: int, y: int, z: int):
        ...
        if x>100: x=x*0.85
        if y>100: y=y*0.85
        radius = math.sqrt(pow(x, 2) + pow(y*0.85, 2))
        ...
        return math.degrees(base_angle), math.degrees(-rear_angle) + 3.120851906137954, 61.64548899867737 - math.degrees(front_angle)
